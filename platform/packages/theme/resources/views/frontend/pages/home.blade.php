            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                <div class="space-y-6">
                    <div class="space-y-4">
                        ...
                        <h2 class="text-3xl lg:text-4xl font-bold text-gray-900">Your Trusted ICT Partner in Tanzania</h2>
                        <p class="text-lg text-gray-600">
                            Founded in 2024, JOBARN GENERAL TRADING COMPANY LTD has quickly established itself as a leading
                            provider of ICT solutions in Tanzania.
                        </p>
                    </div>
                    <x-button as="a" href="/about" class="bg-jobarn-primary hover:bg-jobarn-primary/90">
                        Learn More About Us
                    </x-button>
                </div>
                <div class="relative">
                    <img
                        src="{{ asset('img/comp-network.png') }}"
                        alt="JOBARN About"
        ...
        <div class="container mx-auto px-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach($services as $service)
                    <x-card class="group hover:shadow-lg transition-all duration-300 border-0 shadow-md relative overflow-hidden">
                        <x-card.content class="p-6 space-y-4 relative z-10">
                            <div class="h-12 w-12 bg-jobarn-primary/10 rounded-lg flex items-center justify-center group-hover:bg-jobarn-primary group-hover:text-white transition-colors">
                                <i data-lucide="{{ $service['icon'] }}" class="h-6 w-6 text-jobarn-primary group-hover:text-white"></i>
                            </div>
                ...
                @foreach($products as $product)
                    <x-card class="group hover:shadow-xl transition-all duration-300 overflow-hidden">
                        <div class="relative h-48 overflow-hidden">
                            <img
                                src="{{ $product['image'] ? asset($product['image']) : asset('placeholder.svg') }}"
                                alt="{{ $product['title'] }}"
                ...
                @foreach($partners as $partner)
                    <div class="text-center">
                        <div class="h-16 w-full bg-gray-100 rounded-lg flex items-center justify-center">
                            <span class="text-gray-600 font-semibold text-sm">{{ $partner }}</span>
                        </div>
                    </div>
                @endforeach
            </div>
